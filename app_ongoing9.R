                    ...
                    fluidRow(
                      column(
                        ...
                        fluidRow(
                          column(
                            6,
                            numericInput("user_EPT_taxa", label = NULL, value = 0, min = 0, max = 100, step = 1)
                          ),
                          column(
                            6,
                        ...
                        fluidRow(
                          column(
                            6,
                            numericInput("user_Hydrophyte_total", label = NULL, value = 0, min = 0, step = 1)
                          ),
                          column(
                            6,
...
server <- function(input, output, session) {
    ...
    densi_shade <- eventReactive(densi_values(),{
    ...
    classification <- eventReactive(input$runmodel, {
        Beta_SDAM_GP(
            ...
            user_SubstrateSorting_score = input$user_Substrate_Score,
            user_Sinuosity_score = input$user_Sinuousity,
            user_UplandRootedPlants_score = input$user_UplandRootedPlants_score,
            user_ChannelDimensions_score = input$user_ChannelDimensions_score,
            user_BankWidthMean = bank_mean(),
            user_EPT_taxa = input$user_EPT_taxa,
            user_Hydrophyte_total = input$user_Hydrophyte_total,
            user_PctShade = densi_shade() / 100
        )
    })
